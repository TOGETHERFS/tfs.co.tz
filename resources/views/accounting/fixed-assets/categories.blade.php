    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center mb-6">
            <div>
                ...
                <h1 class="text-2xl font-bold text-gray-900 mt-2">Asset Categories</h1>
            </div>
        </div>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <form action="{{ route('accounting.fixed-assets.categories.store') }}" method="POST">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label for="useful_life_years" class="block text-sm font-medium text-gray-700">Default Useful Life (Years) *</label>
                            <input type="number" step="0.5" min="0.5" name="useful_life_years" id="useful_life_years" value="5" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
                        <div>
                            <label for="salvage_value_percentage" class="block text-sm font-medium text-gray-700">Salvage Value %</label>
                            <input type="number" step="0.01" min="0" max="100" name="salvage_value_percentage" id="salvage_value_percentage" value="0"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
                        <div>
                        ...
                        <div class="flex justify-between items-start">
                            <div>
                                ...
                                <p class="text-xs text-gray-500">
                                    {{ ucwords(str_replace('_', ' ', $category->depreciation_method)) }} / {{ $category->useful_life_years }} years
                                </p>
                            </div>
                            <form action="{{ route('accounting.fixed-assets.categories.destroy', $category) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
