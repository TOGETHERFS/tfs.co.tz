...
class RepaymentController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        // Get all loans for the tenant that can receive repayments
        ...
        if (!empty($validated['schedule_id'])) {
            ...
            if ($schedule) {
                ...
                if ($validated['amount'] > $unpaidForSchedule) {
                    return back()->withErrors([
                        'amount' => 'Repayment amount cannot exceed the installment due amount of TZS ' . number_format($unpaidForSchedule, 2) . ' for this schedule.'
                    ])->withInput();
                }
            }
        }
    ...
    public function searchLoans(Request $request)
    {
        ...
        $loans = Loan::with('client')
                    ->whereIn('status', ['pending', 'approved', 'disbursed', 'active', 'partially_paid'])
    ...
    public function statistics()
    {
        $stats = [
            ...
            'repayments_today' => Repayment::whereDate('payment_date', today())->sum('amount'),
            'repayments_this_month' => Repayment::whereMonth('payment_date', now()->month)
    ...
    private function processSelcomPayment(array $validated, Loan $loan)
    {
        try {
            ...
            return back()->withErrors([
                'payment_method' => 'An error occurred while processing the Selcom payment. Please try again.'
            ])->withInput();
        }
    }
     ...
     */
    public function checkSelcomStatus($transactionId)
    {
        try {
            $selcomTransaction = SelcomTransaction::where('transaction_id', $transactionId)->first();
            ...
            $selcomService = new SelcomPaymentService();
            $status = $selcomService->queryTransactionStatus($selcomTransaction->selcom_transaction_id);
            
            // Update transaction status if payment is successful
            if (isset($status['result']) && $status['result'] === 'SUCCESS' && $selcomTransaction->isPending()) {
                $this->completeSelcomPayment($selcomTransaction, $status);
            }

            return response()->json([
                'status' => 'success',
                'transaction_status' => $selcomTransaction->fresh()->status,
                'transaction_data' => [
                    'id' => $selcomTransaction->transaction_id,
                    'reference' => $selcomTransaction->reference,
                    'amount' => $selcomTransaction->amount,
                    'payment_method' => $selcomTransaction->payment_method,
                    'created_at' => $selcomTransaction->created_at,
                    'status_label' => $selcomTransaction->getStatusLabel(),
                    'status_class' => $selcomTransaction->getStatusClass()
                ],
                'selcom_status' => $status
            ]);

        } catch (\Exception $e) {
            Log::error('Error checking Selcom transaction status', [
